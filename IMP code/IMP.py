...
def main():
  ...
  config.read('C:/Users/redman/Documents/GitHub/SiftingFeatures/inputs/cifar10.ini')
...
#        if st % 100 == 0:
#            ww2, _, _, _ = net.run(sess, batch_xs, batch_ys, data.e_in, data.e_lab, amv, True)
#            qq = ww2[0][0]
#            kk = np.zeros(np.shape(qq)[1])
#            for ii in range (np.shape(qq)[1]):
#                kk[ii] =  np.sum(qq[:, ii]**4) / (np.sum(qq[:, ii]**2)**2)
#            kurt.append(np.mean(kk))
